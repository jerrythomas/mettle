...
UNION ALL
   SELECT t.id
    ...
    WHERE p.task_id                        = t.id
      ...
      AND COALESCE(p.stage_code,'EXTRACT') = 'EXTRACT'
      AND p.status_code               NOT IN ('SUCCESS','FAILURE','KILLED')
   ...
   SELECT t.id
         ...
         ,'Waiting for the precursor task which '
                   ||(CASE WHEN p.id IS NULL THEN
                             'is yet to run.'
                           WHEN p.status_code = 'FAILURE' THEN
                             'has failed and needs to be reprocessed.'
                           WHEN p.status_code = 'KILLED' THEN
...
UNION ALL
   SELECT t.id
    ...
    WHERE t.enabled = TRUE
      ...
      AND TRUE = (SELECT (CASE WHEN COUNT(DISTINCT p.task_id) = COUNT(DISTINCT c.precursor_id) THEN TRUE
                    ...
                    FROM core.chain   c
                         ...
                         LEFT OUTER JOIN logs.process p  ON (    p.task_id        = c.precursor_id
                                                             AND p.status_code    = 'SUCCESS')-- c.status_to_act_on
                   WHERE c.successor_id                           = t.id
                     AND c.enabled                                = TRUE
                     AND c.kind                                  IN ('KEY','CASCADE')
                     AND t.schedule_ts                           >= COALESCE(p.ts_upper_bound,CURRENT_DATE)
                     AND COALESCE(p.ts_upper_bound,CURRENT_DATE) >= CURRENT_DATE
